...
args.printN = parseInt(args.printN);
args.printN = args.printN? Math.max(0, args.printN) : args_dft.printN;
args.printOctave = parseInt(args.printOctave);
args.printOctave = args.printOctave? Math.min(OCTAVES[1], Math.max(OCTAVES[0], args.printOctave)) : args_dft.printOctave;
...
// Contains samples of sine waves for all notes${PERIODS === 0? '' : ` (${PERIODS} periods minimum)`}, octaves ${OCTAVES[0]}-${OCTAVES[1]}, mapped from [0, 1] => [0, 255], at a sample rate of ${SAMPLE_RATE < 1000? `${SAMPLE_RATE.toFixed(2)} Hz` : (SAMPLE_RATE < 1000000? `~${(SAMPLE_RATE / 1000).toFixed(2)} KHz` : `~${(SAMPLE_RATE / 1000000).toFixed(3)} MHz`)}.
//
